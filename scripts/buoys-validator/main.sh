...
if [ $INTERACTIVE -eq 0 ] 
    ...
    #infilenc="$INDIR/$MODEL/OUNP-LSF-MEDNORD-2012_tab.nc"
    #infilenc="$INDIR/$MODEL/OUNP-LSF-MEDNORD-2012_tab-mar.nc"
    # infilenc="$INDIR/$MODEL/OUNP-LSF-MEDNORD-2012_tab-allyear.nc"
    extension=ZWND6
    infilenc="$INDIR/$MODEL/OUNP-LSF-MEDNORD-2012_ALLYEAR-$extension.nc"
    infilenc="$INDIR/$MODEL/OUNP-LSF-MEDNORD-2011_tab-ZWND6.nc"
...
# stations
stations="Espiguette Sete Leucate Banyuls MeteoFranc TOULON MARSEILLE CAMARGUE NICELARGE LION PORQUEROLLES NICE"
# stations="MeteoFranc"
...
sed "s/	/,/g" $WORKDIR/$station-hs-joined.csv > $OUTDIR/$station-hs$extension.csv
...
sed "s/	/,/g" $WORKDIR/$station-dir-joined.csv > $OUTDIR/$station-dir$extension.csv
...
sed "s/	/,/g" $WORKDIR/$station-dirpeak-joined.csv > $OUTDIR/$station-dirpeak$extension.csv
...
sed "s/	/,/g" $WORKDIR/$station-fp-joined.csv > $OUTDIR/$station-fp$extension.csv
...
### WIND COMPARISON FOR METEO FRANCE LION BUOY ###
